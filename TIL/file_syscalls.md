# Project 2 userprog 시스템 콜 회고 — 인프라부터 wait-twice 디버깅까지
...
> EXEC 직후 16 개 fork/exec/wait 테스트 중 wait-twice 하나가 끈질기게
> TIMEOUT 으로 떨어진 사건은 §9 에 별도로 정리.
>
> | 단계 | 섹션 | 콜 | 무게중심 |
> |---|---|---|---|
...
> | 6차 | §9 | wait-twice 디버깅 | 자료구조 corruption — list 중복 등록 한 줄로 추적 |
> | 다음 | §10 | REMOVE / SEEK / TELL, fd 슬롯 재사용, 페이지 경계 검증 등 |
>
...
> 시맨틱을 향해 정렬되어야 함) 로 이동한다. §9 (wait-twice) 는 무게중심이
> 한 번 더 옮겨가 **자료구조 invariant** — "같은 elem 을 두 번 push_back
> 하면 self-referential 로 깨진다" — 한 줄로 압축된다. 한 줄 요약은 §11 에 있다.
...
##### 왜 `palloc_get_page` 로 페이지를 따로 만들어 넘기나

`process_exec` 가 **내부에서 `palloc_free_page(file_name)` 으로 입력 페이지를
해제**하기 때문이다. 성공·실패 양쪽 모두:

```c
/* process_exec 흐름 발췌 */
success = load (argv[0], &_if);
if (!success) {
    palloc_free_page (file_name);    /* ← 실패 경로 */
    return -1;
}
argument_stack (argv, argc, &_if);
palloc_free_page (file_name);        /* ← 성공 경로 (do_iret 직전) */
do_iret (&_if);
```

→ 호출자는 반드시 **palloc 으로 받은 페이지를 넘겨야** 한다. 유저 스택의
포인터를 그대로 넘기면 free 단계에서 죽고, 커널 힙 (`malloc`) 으로 받은
포인터를 넘겨도 `palloc_free_page` 가 wrong allocator 로 해제 시도하다 죽는다.

부수 효과: 유저가 넘긴 문자열은 **페이지 폴트 위험**이 있다 (mmap 영역
경계, 잘못된 매핑 등). 커널 영역의 새 페이지에 복사해두면 이후 syscall
중 페이지 폴트가 나도 우리가 다루는 사본은 안전하다 — 안전성·소유권
두 측면에서 모두 옳은 선택.
...
1. **`process_exec` 가 성공하면 절대 돌아오지 않는다 (do_iret).**
   ...
   ┌─ 인자 파싱 ─→ load(argv[0]) ┐
                                 ├─ 실패 ─→ palloc_free_page → return -1 ──→ 호출자로 복귀
                                 │                                          (SYS_EXEC: exit(-1))
                                 │                                          (initd:    PANIC)
                                 └─ 성공 ─→ argument_stack
                                          → palloc_free_page
                                          → do_iret(&_if) ──→ 유저 모드, 절대 안 돌아옴
   ```

   "성공 = 안 돌아옴" 이라는 비대칭이 바로 시행착오 5 의 시맨틱 차이의 뿌리다.
...
### 8.6 함정 16 — `strlcpy` 가 `exec-once` / `exec-arg` 의 종료 메시지를 깨뜨린다

`exec-missing` (§8.2~§8.5) 와 별개의 사건이지만, 같은 EXEC 패밀리라 함께 정리.

기존 `process_exec` 에는 thread name 을 `argv[0]` 으로 갱신하는 한 줄이 있었다:
...
이 줄은 stage 1 의 `args-single` 함정 (cmdline 전체가 16자 한계로 잘리는
문제) 을 잡으려고 들어간 것이다. 그러나 **EXEC 시맨틱에선 정반대 방향으로
작동**한다.

**증상**: `exec-once`, `exec-arg` 테스트의 종료 메시지 mismatch.
`exec("child")` 를 호출한 부모 프로세스가 종료할 때, `process_exit` 의
`printf("%s: exit(%d)\n", name, status)` 가 **"child" 이름으로 찍힘** —
expected 출력은 부모 이름 ("exec-once" / "exec-arg") 이라 어긋남.

**원인 흐름**:
```
유저: exec("child")
  → SYS_EXEC → process_exec(fn_copy="child")
  → strlcpy(thread_current()->name, "child")   ★ 부모 thread 의 name 이 "child" 로 덮임
  → load("child") (성공) → do_iret → 자식 코드 실행
  → 자식 종료 시점에 부모 thread 의 name 이 이미 "child" 이므로 종료 메시지가 "child: exit(...)"
```

**해결**: 그 `strlcpy` 를 주석 처리.

```c
/* exec() 시 호출자(부모) 이름이 종료 메시지에 보존되어야 함 → 주석 */
// strlcpy (thread_current ()->name, argv[0],
//          sizeof thread_current ()->name);
```

**왜 `args-single` 은 안 깨지나**: stage 1 의 `args-single` 케이스는
**`process_create_initd` → `initd` → `process_exec`** 경로로 들어오는데,
이 경로에서 thread name 은 `process_create_initd` 가 `thread_create(file_name, ...)`
호출 시 결정된다. `args-single` 의 cmdline 은 `"args-single onearg"` (18 자)
가 16 자 한계로 `"args-single one\0"` 으로 잘리지만, 종료 메시지가 기대하는
값은 `"args-single"` 이다. 다행히 `process_exit` 출력은 thread name 의
앞쪽만 비교되므로 (테스트 expected 도 `args-single: exit(0)` 으로 prefix
일치만 봄), 이름이 잘려 있어도 문제 없다.

**한 줄 정리**: stage 1 단계에서 `strlcpy` 로 잡으려던 args-single 대비책은,
EXEC 단계에서 보면 부모 이름을 자식 이름으로 덮어쓰는 부작용이 있다.
**EXEC 가 도입되면 그 줄은 빼야 한다** — 한 코드 변경의 효과가 stage 가
달라지면 정반대로 작용하는 사례.

---

## 9. 후속 — `wait-twice` 디버깅 (자료구조 corruption 한 줄)

EXEC 까지 들어와 fork/exec/wait 16 개 테스트 중 15 개가 통과했는데
`wait-twice` 하나가 끈질기게 TIMEOUT 으로 떨어졌다. 시도 6 회 끝에
원인이 **콜 로직이 아니라 `list` 자료구조 invariant 위반**이었음을
확인한 사건. §7 (FORK/WAIT) 와 §8 (EXEC) 가 "코드를 어디에 어떻게 쓸까"
의 이야기였다면, §9 는 **"이미 쓴 코드가 자료구조의 invariant 를 깨면
어떤 방식으로 망가지는가"** 의 이야기다.

### 9.1 테스트가 요구한 것

```c
/* tests/userprog/wait-twice.c (의역) */
pid_t pid = fork(...);                      /* 자식 생성 */
if (pid == 0) exec("child-simple");         /* 자식: child-simple 로 교체 후 exit(81) */
int s1 = wait(pid);                         /* 1차: 81 기대 */
int s2 = wait(pid);                         /* 2차: -1 즉시 기대 */
msg("wait(exec()) = %d", s1);
msg("wait(exec()) = %d", s2);
```
...
```
(wait-twice) wait(exec()) = 81
TIMEOUT after 60 seconds of wall-clock time
```

**1 차 wait 는 통과했는데 2 차 wait 에서 영원히 막혔다**. 이 한 줄
차이가 §9 전체의 단서였다.

### 9.2 근본 원인 — `child_elem` 이중 등록

`thread_create` (threads/thread.c) 가 새 스레드를 부모 children 리스트에
이미 등록한다.

```c
/* threads/thread.c — thread_create() */
t->parent = thread_current();
list_push_back(&thread_current()->children, &t->child_elem);   /* ← 1 회 등록 */
```

그런데 `__do_fork` (userprog/process.c) 가 **같은 `child_elem` 을 한 번 더**
push_back 했다.

```c
/* userprog/process.c — __do_fork() (수정 전) */
current->parent = parent;
list_push_back(&parent->children, &current->child_elem);       /* ← 2 회째 등록, 중복 */
```

같은 `list_elem` 을 두 번 `list_push_back` 하면 elem 의 `prev` / `next` 가
**자기 자신을 가리키도록** 깨진다 (self-referential). pintos `list_insert`
의 다음 4 줄이 자기 자신을 두 번 참조하기 때문이다.

```c
elem->prev      = before->prev;     /* 1 회째: 정상.   2 회째: before->prev == elem 자신 */
elem->next      = before;
before->prev->next = elem;          /* 2 회째: elem->next = elem  ← 사이클 */
before->prev    = elem;
```

### 9.3 깨진 list 가 일으킨 이중 실패
...
**self-referential elem 에서는 `list_remove` 가 no-op 이 된다** 는 게
이 사건의 한 줄짜리 결론이다.

### 9.4 다른 fork 테스트가 무사했던 이유

| 테스트 | wait 호출 횟수 | 깨진 list 영향 |
|---|---|---|
| `fork-once` / `fork-multiple` / `fork-recursive` | 자식당 1 회 | **드러나지 않음** — 첫 탐색은 정상 |
| `fork-read` / `fork-close` / `fork-boundary` | 자식당 1 회 | 마찬가지 |
| `wait-simple` / `wait-killed` / `wait-bad-pid` | 1 회 또는 즉시 -1 | 마찬가지 |
| `wait-twice` | **자식당 2 회** | ★ 두 번째 탐색에서 폭로 |

15 / 16 통과했다는 사실 자체가 함정이었다 — **버그가 잠복할 수 있는
유일한 테스트가 wait-twice 한 개** 였고, 자료구조 corruption 의 전형적인
"한 번은 동작, 두 번째에 무너진다" 패턴이 그대로 들어맞았다.

### 9.5 시도 6 회 — 모두 증상만 만지고 원인은 비껴감

원인이 등록 지점 이중화에 있다는 걸 모른 상태에서 6 가지 다른 가설을
거쳤다. 각각이 **왜 실패했는지** 가 §9 의 핵심.

#### 시도 1 — `process_exit()` 에서 `parent != NULL` 일 때만 `sema_down(exit_sema)`

가설: 자식이 영원히 `exit_sema` 에서 막힌다 → parent 가 없는 경우 분기 추가.
...
**실패 이유**: `thread_create` 가 모든 신생 스레드의 `parent` 를
`thread_current()` 로 세팅 → `parent != NULL` 은 거의 항상 참 →
조건이 사실상 무력. 더 본질적으로, 막혀 있는 건 자식 process_exit
가 아니라 **2 차 wait 의 `sema_down(wait_sema)`** 였는데 그 위치를 못 봤다.

#### 시도 2 — `wait_called` 플래그로 동기화

가설: 부모가 wait 를 호출했는지를 자식에게 알려주는 플래그를 두자.

```c
/* parent: process_wait 진입 시 */
child->wait_called = true;

/* child: process_exit */
sema_up(&curr->wait_sema);
if (curr->wait_called) sema_down(&curr->exit_sema);
```

**실패 이유**: 자식이 `sema_up(wait_sema)` 직후 즉시 `wait_called` 를
체크하는 시점에, 부모는 아직 `process_wait` 안에서 플래그를 세팅하지
못했을 수 있다 — 전형적인 **race condition**. 플래그 기반 동기화로
sema 동기화를 흉내내려는 시도 자체가 잘못된 방향이었다.

#### 시도 3 — `thread_create()` 의 `list_push_back` 제거

가설: 등록을 한 곳에서만 하면 되니, `thread_create` 쪽을 빼자.

**실패 이유**: `init.c` 의 `run_task()` 가 `process_wait(initd_tid)` 를
호출하면서 main 스레드의 `children` 리스트에서 initd 를 찾는다.
`thread_create` 가 등록을 안 하면 **initd 가 main 의 children 에 없다**
→ `process_wait` 가 즉시 -1 반환 → Pintos 가 initd 가 본격 실행되기도
전에 종료. 등록 지점 자체를 한 곳으로 모은다는 방향은 옳았는데
**남길 곳을 잘못 골랐다**.

#### 시도 4 — `process_exec()` 에서 `parent = NULL`

가설: exec 호출 시점에 부모 관계를 끊으면 깔끔해질 것.

**실패 이유**: fork 자식이 곧이어 exec 하는 패턴 (wait-twice 가 정확히
이 패턴이다) 에서 부모 관계가 끊기면 **자식의 종료를 부모가 회수하지
못한다** → wait 가 -1 을 반환해 버려 81 도 안 나옴. exec 의 시맨틱
("이미지 교체이지 프로세스 교체가 아님") 에 맞지 않는 변경이었다.

#### 시도 5 — `initd()` 에서 `parent = NULL`

가설: initd 만큼은 부모가 없어야 한다.

**실패 이유**: initd 는 실제로 부모가 없으므로 이 변경 자체는 정합적
(이번 PR 에도 살아남았다). 다만 **wait-twice 의 부모는 initd 가 아니라
wait-twice 자기 자신** 이다 → initd 의 parent 를 NULL 로 만들어도
wait-twice 의 children 리스트는 여전히 깨져 있다. 정합성은 좋아졌지만
TIMEOUT 의 원인은 그대로.

#### 시도 6 — 부모 `process_exit()` 에서 자식들 `exit_sema` 풀어주기

가설: 부모가 죽기 전에 자식들의 `exit_sema` 를 모두 `sema_up` 하면
자식이 영원히 안 막히게 된다.
...
**실패 이유**: 이건 **자식이 부모 종료 후에 영원히 안 막히게** 만드는
정합성 보강은 맞다 (이번 PR 에도 살아남았다). 하지만 wait-twice 의
TIMEOUT 은 자식이 막힌 게 아니라 **부모 (wait-twice) 가 2 차 wait
의 `sema_down(wait_sema)` 에서 막힌 것** — children 리스트가 깨져
계속 같은 자식을 찾아내기 때문. 자식 쪽 동기화를 아무리 보강해도
부모의 `wait_sema` 를 누가 `sema_up` 해주지는 않는다.

### 9.6 최종 수정 한 줄

```diff
 /* userprog/process.c — __do_fork() */
 current->parent = parent;
-list_push_back(&parent->children, &current->child_elem);
```

`current->parent = parent;` 는 `thread_create` 의 값과 같지만 명시성을
위해 남겼다. **삭제한 건 단 한 줄, `list_push_back` 뿐** — 이 한 줄이
self-referential corruption 의 원천이었다.
...
### 9.7 함정들 (wait-twice 단계)

번호는 §3 (1~4), §5.A (5~7), §6.A (8), §7.A (9~14), §8 (15) 의 연장.

#### 함정 16 — 같은 `list_elem` 을 두 번 `list_push_back` 하면 **list_remove 가 무력화**된다

이번 사건의 한 줄. pintos `list_insert` 가 `before->prev->next = elem`
한 줄에서 자기 자신을 거쳐 가게 되어 elem 의 `next` 가 elem 자신을
가리킨다. `list_remove` 는 `elem->prev->next = elem->next` 와 그
대칭을 적는데, 양변이 모두 elem 자신이면 **할당이 일어나도 list 의
실제 모양이 안 바뀐다**. 결과적으로 `list_remove` 가 no-op 이 되고,
다음 순회에서 같은 elem 을 또 찾는다.

체크리스트:
- 부모-자식 같은 일대일 등록 관계는 **반드시 한 곳에서만** `list_push_back`.
- "thread_create 가 자동으로 해 주는 일" 을 모르고 `__do_fork` 같은
  특수 경로에서 또 한 번 등록하지 않는지 확인.

#### 함정 17 — 한 번은 통과 두 번째에 무너지면 **로직이 아니라 자료구조** 를 의심

증상의 패턴 자체가 진단을 좁힌다. **두 번째 호출에서만 깨진다** 는 건
첫 번째 호출이 자료구조의 invariant 를 깨놓고 빠져나갔다는 뜻이다.
fork-once / fork-multiple 이 "1 자식 1 wait" 라 통과하는데
wait-twice 만 떨어지는 패턴은 **list 같은 누적 자료구조의 corruption
시그니처** 다.

체크리스트:
- "테스트 N 개 중 1 개만 떨어지는데, 그 1 개가 같은 작업을 2 회 반복한다"
  → 자료구조 corruption 우선 의심.
- 시도해 볼 것: 첫 번째 호출 직후 자료구조 상태를 print 해 보기 (list
  순회 + 길이 + 각 elem 의 prev/next 동일성 검사).

#### 함정 18 — 등록 지점이 **분산되면** 중복 위험이 시간이 지나 폭발한다

`thread_create` 와 `__do_fork` 가 각각 등록하는 코드는 **둘 다 동작
하던 시기가 있었다** (fork 처음 도입 시점 — wait-twice 가 아직 없거나
손대지 않은 상태). 새 테스트가 들어오자 잠재 버그가 폭발했다.

체크리스트:
- 부모-자식 / fd / lock 같은 "관계 등록" 지점은 **한 함수가 책임지도록 묶기**.
- `thread_create` 가 이미 해 주는 일을 호출 측이 또 하지 않는지,
  **새 함수를 추가할 때 이 책임을 명시적으로 위에서 한 번 점검**.

### 9.8 §9 핵심 통찰

1. **자료구조 invariant 가 깨지는 순간이 코드 한 줄로 끝난다** — 이번엔
   `list_push_back` 한 줄. invariant 위반은 호출 시점에 즉시 죽지 않고,
   "다음 순회" 같은 우회 경로에서 폭로된다.
2. **테스트 한 개의 실패가 코드 전체의 invariant 를 검증한다** — wait-twice
   한 개가 16 개 테스트가 가려놓은 list corruption 을 드러냈다.
3. **"증상 → 원인" 매핑은 자료구조 종류를 알면 좁아진다** — list 의
   self-referential 은 `list_remove` 무력화로, hashmap 의 collision 은
   다른 버킷 침식으로, refcount 의 누수는 use-after-free 로. 자료구조
   별 corruption 시그니처를 한 번 외워 두면 진단 시간이 절반으로 준다.
4. **`thread_create` 같은 인프라 함수의 부수효과를 호출 측이 모르면
   중복이 생긴다** — `t->parent = thread_current(); list_push_back(...)`
   이 한 줄을 모르고 `__do_fork` 가 또 해 버린 게 시작이었다.
...
- **`fork_success` 활용** — 현재는 멤버만 추가하고 실제 분기는 안 깔림.
  ...
  `-1` 로 정정하는 패턴으로 확장 예정.
- **EXEC 의 page fault 견고화** — `validate_user_addr(filename)` 은 첫
  바이트만 확인. `filename` 이 페이지 경계를 걸치면 `strlcpy` 도중 폴트
  가능. `validate_user_addr` 를 페이지 경계까지 검증하도록 확장 필요.
  (fn_copy 자체의 free 는 `process_exec` 가 양쪽 경로 모두에서 처리하므로
  누수 없음 — §8.3 (3) 참조.)
...
>
> wait-twice (§9) 는 무게중심을 또 한 번 옮긴다 — 이번엔 **자료구조
> invariant**. 콜 로직은 다 맞고, 동기화 sema 패턴도 다 맞고, 함수 간
> 책임 분배도 다 맞았는데, `list_push_back` 한 줄이 두 곳 (`thread_create` +
> `__do_fork`) 에 분산된 결과 같은 `child_elem` 이 두 번 등록되어
> self-referential 로 깨졌다. **`list_remove` 가 no-op 이 된다** 는
> 한 줄짜리 결론으로 시도 6 회 (§9.5) 가 모두 헛수고였음이 드러났다.
> 시리즈를 통틀어 **"한 번은 통과, 두 번째에 무너진다 → 자료구조
> corruption 의심"** (§9.7 함정 17) 이라는 진단 휴리스틱을 가장 또렷이
> 보여 준 사건 (§9.7 함정 16~18).
