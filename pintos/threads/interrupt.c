...
void intr_init(void)
{
	...
	/**
	 * PIC를 초기화하고 IRQ 번호를 새 벡터 범위로 재매핑
	 * PIC : 8259A Programmable Interrupt Controller
	 * 옛날 PC에서 타이머, 키보드 같은 외부 IRQ를 CPU에 전달하는 장치
	 */
	pic_init();
	...
	/**
	 * 모든 IDT 엔트리 초기화
	 * 즉 256개 벡터 각각에 대해 "이 벡터가 오면 이 stub으로 들어가라"를 세팅
	 */
	for (i = 0; i < INTR_CNT; i++)
	{
		/**
		 * intr_stubs란
		 * 각 벡터마다 공통 진입용 작은 어셈블리 stub가 하나씩 있음
		 *
		 * 1. 벡터별 stub 진입
		 * 2. vec_no, error_code 같은 공통 포맷 맞춤
		 * 3. list_entry로 점프
		 * 4. 거기서 레지슽터 저자
		 * 5. intr_handler() 호출
		 *
		 * 즉 IDT는 직접 C 함수로 바로 들어가는 게 아니라,
		 * 먼저 stub -> 공통 엔트리 -> C handler 구조로 들어감
		 *  */
	 ...
	 * -> CPU에게 "벡터 번호 N번이 오면 어느 함수 주소로 들어가고, 어떤 게이트 타입으로 처리할지를 알려주는 표"
	 */
	lidt(&idt_desc);
	...
	/**
	 * - 디버깅용 이름 붙이기 -
	 * 넓게 보면 전부 인터럽트 벡터 엔트리로 볼 수 있음
	 * 정확히는 예외 + 외부 하드웨어 + 소프트웨어적으로 쓸 수 있는 나머지 벡터가 섞여 있음
	 * 0x00 ~ 0x1f (0 ~ 31)
	 * - 주로 CPU 예외 / 트랩
	 * - ex. 0으로 나누기, invalid opcode, page fault
	 * 0x20 ~ 0x2f (32 ~ 47)
	 * - 외부 하드웨어 인터럽트
	 * - PIC가 보내는 IRQ 0 ~ 15
	 * ex. timer, keyboard
	 *
	 * 그 밖의 0x30 ~ 0xff
	 * - 아직 안 쓰거나, 나중에 소프트웨어 인터럽트/시스템콜/확장용으로 쓸 수 있는 벡터
	 */
	intr_names[0] = "#DE Divide Error"; // 0 나누기 예외
